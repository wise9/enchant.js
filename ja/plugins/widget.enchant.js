 ...
 * {@link enchant.Game#transitionPush}のアニメーションが終了した際に発生する.
 * @type {String}
 */
 ...
 * {@link enchant.Game#transitionPop}のアニメーションが終了した際に発生する.
 * @type {String}
 */
...
var distribute = function(value, div) {
    if (typeof div == 'array') {
        ...
        ratio.forEach(function(n) { sum += n; });
        quo = value / sum;
        ...
        ret.forEach(function(n) { retSum += n; });
...
enchant.widget.Ninepatch = enchant.Class.create(enchant.Surface, {
    ...
    _slicedraw: function(img) {
        ...
        var verQueue = this._verScStore.concat(this._verNoScStore).sort(function(a, b) { return a[0] - b[0]; });
        var horQueue = this._horScStore.concat(this._horNoScStore).sort(function(a, b) { return a[0] - b[0]; });
