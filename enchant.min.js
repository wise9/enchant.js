...
if(d){var h=window.getComputedStyle(d),a=parseInt(h.width),b=parseInt(h.height);a&&b?this.scale=Math.min(a/this.width,b/this.height):(d.style.width=this.width+"px",d.style.height=this.height+"px");for(;d.firstChild;)d.removeChild(d.firstChild);d.style.position="relative";h=d.getBoundingClientRect();this._pageX=Math.round(window.scrollX+h.left);this._pageY=Math.round(window.scrollY+h.top)}else d=document.createElement("div"),d.id="enchant-stage",d.style.width=window.innerWidth+"px",d.style.height=
window.innerHeight+"px",d.style.position="absolute",document.body.firstChild?document.body.insertBefore(d,document.body.firstChild):document.body.appendChild(d),this.scale=Math.min(window.innerWidth/this.width,window.innerHeight/this.height),this._pageY=this._pageX=0;if(!this.scale)this.scale=1;d.style.fontSize="12px";d.style.webkitTextSizeAdjust="none";this._element=d;this.fps=30;this.frame=0;this.ready=null;this.running=!1;this.assets={};var j=this._assets=[];(function q(a){a.assets instanceof Array&&
[].push.apply(j,a.assets);for(var b in a)a.hasOwnProperty(b)&&Object.getPrototypeOf(a[b])==Object.prototype&&q(a[b])})(enchant);this._scenes=[];this.currentScene=null;this.rootScene=new enchant.Scene;this.pushScene(this.rootScene);this.loadingScene=new enchant.Scene;this.loadingScene.backgroundColor="#000";var i=0.9*this.width|0,o=0.3*this.width|0,l=0.05*i|0,d=new enchant.Sprite(i,o);d.x=(this.width-i)/2;d.y=(this.height-o)/2;var k=new enchant.Surface(i,o);k.context.fillStyle="#fff";k.context.fillRect(0,
0,i,o);k.context.fillStyle="#000";k.context.fillRect(l,l,i-2*l,o-2*l);d.image=k;var f=0,g=0;this.addEventListener("progress",function(a){f=a.loaded/a.total});d.addEventListener("enterframe",function(){g*=0.9;g+=0.1*f;k.context.fillStyle="#fff";k.context.fillRect(l,0,(i-2*l)*g,o)});this.loadingScene.addChild(d);this._soundID=this._surfaceID=this._mousedownID=0;this._intervalID=null;this.input={};this._keybind={};this.keybind(37,"left");this.keybind(38,"up");this.keybind(39,"right");this.keybind(40,
...
e.start()},!0);e.pushScene(a);return}var h={},b=this._assets.filter(function(a){return a in h?!1:h[a]=!0}),j=0,c=0,i=b.length;for(;c<i;c++)this.load(b[c],function(){var a=new enchant.Event("progress");a.loaded=++j;a.total=i;e.dispatchEvent(a);j==i&&(e.removeScene(e.loadingScene),e.dispatchEvent(new enchant.Event("load")))});this.pushScene(this.loadingScene)}else this.dispatchEvent(new enchant.Event("load"));this.currentTime=Date.now();this._intervalID=window.setInterval(function(){e._tick()},1E3/
