...
enchant.ENV.TOUCH_ENABLED)window.setTimeout(function(){c.dispatchEvent(new enchant.Event("load"))},0);else if(!enchant.ENV.USE_FLASH_SOUND&&g.canPlayType(b)){g.src=a;g.load();g.autoplay=false;g.onerror=function(){throw Error("Cannot load an asset: "+g.src);};g.addEventListener("canplaythrough",function(){c.duration=g.duration;c.dispatchEvent(new enchant.Event("load"))},false);c._element=g}else if(b=="audio/mpeg"){var h=document.createElement("embed"),d="enchant-audio"+e._soundID++;h.width=h.height=
1;h.name=d;h.src="sound.swf?id="+d+"&src="+a;h.allowscriptaccess="always";h.style.position="absolute";h.style.left="-1px";c.addEventListener("load",function(){Object.defineProperties(h,{currentTime:{get:function(){return h.getCurrentTime()},set:function(a){h.setCurrentTime(a)}},volume:{get:function(){return h.getVolume()},set:function(a){h.setVolume(a)}}});c._element=h;c.duration=h.getDuration()});e._element.appendChild(h);enchant.Sound[d]=c}else window.setTimeout(function(){c.dispatchEvent(new enchant.Event("load"))},
0);return c};window.addEventListener("message",function(a){try{var b=JSON.parse(a.data);if(b.type=="event")enchant.Game.instance.dispatchEvent(new Event(b.value));else if(b.type=="debug")switch(b.value){case "start":enchant.Game.instance.start();break;case "pause":enchant.Game.instance.pause();break;case "resume":enchant.Game.instance.resume();break;case "tick":enchant.Game.instance._tick()}}catch(d){}},false);enchant.Sound.enabledInMobileSafari=false})();
